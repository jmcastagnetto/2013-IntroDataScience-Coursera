...
def compute_hands():
    ...
    outfile = bz2.BZ2File('hands-json.bz2', 'w')
...
def mapper(record):
    record = record.encode('utf8').translate(None, '"')
    ...
    for i in range(0, 11):
        if (counts[straightrunfaces[i]] and
            ...
            break
    for i in range(0, 10):
        if (counts[straightrunfaces[i+1]] and
                counts[straightrunfaces[i+2]] and
                counts[straightrunfaces[i+3]] and
                counts[straightrunfaces[i+4]] and
                not is_straight):
            is_4cardstraight = True
            break
...
def main():
    try:
        inputdata = bz2.BZ2File('hands-json.bz2')
    except IOError:
        compute_hands()
        inputdata = bz2.BZ2File('hands-json.bz.bz2')
